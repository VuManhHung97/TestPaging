    ...
    override fun loadSingle(
        ...
        return Single.just(loadType)
            ...
            .map {
                when (it) {
                    LoadType.REFRESH -> {
                        Log.d("okhttp RemoteMediator", "REFRESH")
                        val remoteKeys = getRemoteKeyClosestToCurrentPosition(state)

                        remoteKeys?.nextKey?.minus(1) ?: 1
                    }
                    LoadType.PREPEND -> {
                        Log.d("okhttp RemoteMediator", "PREPEND")
                    ...
                    LoadType.APPEND -> {
                        Log.d("okhttp RemoteMediator", "APPEND")
            ...
            .flatMap { page ->
                if (page == INVALID_PAGE) {
                    ...
                    service.popularMovieRx(page)
                        .map { mapper.transform(it, locale) }
                        .map { insertToDb(page, loadType, it) }
                        .map<MediatorResult> {
                            Log.d("okhttp RemoteMediator", "MediatorResult.Success(${it.endOfPage})")
                            MediatorResult.Success(endOfPaginationReached = it.endOfPage)
                        }
                        .onErrorReturn {
                            Log.d("okhttp RemoteMediator", "MediatorResult.Error(${it.localizedMessage})")
